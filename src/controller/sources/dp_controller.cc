...
std::string s21::DpController::DepositData(bool capitalization, int pay_idx,
  ...
  s21::Deposit::F repl_freq =
      repl_idx == 0 ? Deposit::F::NO : Deposit::F(repl_idx + 2);
  s21::Deposit::F rem_freq =
      rem_idx == 0 ? Deposit::F::NO : Deposit::F(rem_idx + 2);

  std::string result =
      d->CalculateProfit(pay_freq, repl_freq, rem_freq, capitalization);
