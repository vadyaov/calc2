...
TEST(deposit_test, t1) {
  ...
  std::string out = d.CalculateProfit(Deposit::F::QUARTER, Deposit::F::NO,
                                      Deposit::F::NO, false);
  std::string expected =
      " INCOME ---> 16450.01"
      "\nDEPOSIT --> 350000"
...
TEST(deposit_test, t2) {
  ...
  Deposit d(Amount(500000.0, 20000, 0), Rate(4.7, 30),
            Days(str_pair(start, end)));
  std::string out = d.CalculateProfit(Deposit::F::MONTH1, Deposit::F::MONTH6,
                                      Deposit::F::NO, false);
  std::string expected =
      " INCOME ---> 75532.22"
      "\nDEPOSIT --> 600000"
...
TEST(deposit_test, t3) {
  std::string start = "12.11.2023", end = "17.10.2026";
  Deposit d(Amount(500000.0, 20000, 15000), Rate(4.7, 30),
            Days(str_pair(start, end)));
...
TEST(deposit_test, e1) {
  ...
  EXPECT_THROW(
      {
        try {
          Deposit d(Amount(-350000.0, 0, 0), Rate(4.7, 30),
                    Days(str_pair(start, end)));
...
TEST(deposit_test, e2) {
  ...
  EXPECT_THROW(
      {
        try {
          Deposit d(Amount(350000.0, -1, 0), Rate(4.7, 30),
                    Days(str_pair(start, end)));
        } catch (const std::logic_error& e) {
          EXPECT_STREQ("Negative value error", e.what());
          throw;
